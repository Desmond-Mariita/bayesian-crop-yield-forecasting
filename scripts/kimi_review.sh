...
# Run an AGENTIC review with Kimi (Moonshot) by driving the Claude Code harness against
# Moonshot's Anthropic-compatible endpoint. Kimi reads/greps the repo itself (like Claude/
# Codex), then files a provenance-stamped report. Needs MOONSHOT_API_KEY (env or ~/.bashrc).
#
# Usage:
#   scripts/kimi_review.sh -o reviews/kimi/<slug>.md [opts] <input-file>... <<'PROMPT'
#   ...prompt text...
#   PROMPT
#
# Options:
...
command -v claude >/dev/null || { echo "claude CLI not found on PATH (it's the harness for Kimi here)" >&2; exit 3; }
...
# Resolve the Moonshot key (env or ~/.bashrc; wrapper works in non-interactive shells).
KEY="${MOONSHOT_API_KEY:-$(grep -oP 'MOONSHOT_API_KEY="\K[^"]+' "$HOME/.bashrc" 2>/dev/null | tail -1 || true)}"
[ -n "$KEY" ] || { echo "MOONSHOT_API_KEY not set and not found in ~/.bashrc" >&2; exit 3; }

# Point the Claude Code harness at Moonshot's Anthropic-compatible endpoint, and force the
# Kimi model for every tier Claude Code might request. Unset any real Anthropic key so it
# can't override the endpoint.
...
{
  printf '# %s\n\n' "$(basename "${OUT%.md}")"
  printf '> Generated by Kimi (`%s`) via Moonshot Anthropic-compatible endpoint + Claude Code harness on %s.\n' \
    "$MODEL" "$(date '+%F %T %Z')"
  printf '> Inputs (%d): %s\n\n---\n\n' "${#INPUTS[@]}" "${INPUTS[*]}"
  printf '%s\n' "$clean"
} > "$OUT"
